...
const nodemailer   = rerequire('dotenv').config();
...
async function initDB() {
  await pool.query(`
    ...
    INSERT INTO settings (key, value) VALUES
      ...
      ('payment_btc_note', 'Send only BTC to this address. Include reference code if your wallet supports memos.'),
...
app.get('/api/appointments/receipt.pdf', async (req, res) => {
  ...
  try {
    ...
    doc.fontSize(8).fillColor('#999')
       .text('Thank you for choosing Ink & Iron.',
  ...
  try {
    ...
    transporter.sendMail({
      ...
      html: emailWrapper('Appointment Confirmed', `
        ...
        <table style="width:100%;border-collapse:collapse;margin:1.5rem 0;">
          <tr><td style="padding:6px 0;color:#999;">Receipt #</td>
              <td style="padding:6px 0;text-align:right;">${b.receipt_number}</td></tr>
          <tr><td style="padding:6px 0;color:#999;">Amount paid</td>
              <td style="padding:6px 0;text-align:right;">
                $${parseFloat(b.payment_amount || 0).toFixed(2)}
              </td></tr>
          <tr><td style="padding:6px 0;color:#999;">Method</td>
              <td style="padding:6px 0;text-align:right;">
                ${methodLabel(b.payment_method)}
              </td></tr>
          <tr><td style="padding:6px 0;color:#999;">Appointment date</td>
              <td style="padding:6px 0;text-align:right;">
                ${b.preferred_date
                  ? new Date(b.preferred_date).toLocaleDateString('en-US')
                  : 'To be confirmed'}
